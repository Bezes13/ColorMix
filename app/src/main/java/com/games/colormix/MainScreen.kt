                    ...
                    for (i in gameField.indices) {
                        Column(verticalArrangement = Arrangement.spacedBy(VerticalPadding)) {
                            for (j in gameField[i].indices) {
                                Box( modifier = Modifier.size(FieldSize), contentAlignment = Alignment.Center) {
                                    if(Pair(i,j) == animateAt){
                                        Explosion(progress = progress, Color.Red)
                                        println(progress)
                                    }
                                }
