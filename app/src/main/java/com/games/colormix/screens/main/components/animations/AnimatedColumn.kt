...
private data class WithVisibility<T>(
    val data: T,
    val state: VisibilityState = VisibilityState.Initial
)
        ...
        if (total.isNotEmpty()) {
            internalList.run {
                for (removed in removedItems) {
                    this[removed.index] = this[removed.index].copy(state = VisibilityState.ToClose)
                }
                for (added in addedItems) {
                    add(added.index, WithVisibility(added.value, VisibilityState.ToPopUp))
                }
            }
